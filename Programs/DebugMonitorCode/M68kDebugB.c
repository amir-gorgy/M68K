...
void ProgramFlashChip(void)
{
    ...
    // TODO : then verify by reading it back and comparing to memory <- Check Function Below
    //

    unsigned char *RamPtr;
    unsigned char Databyte, x;
    unsigned char FMemAddrByte1, FMemAddrByte2, FMemAddrByte3;
    int FAddr = 0;
    int count = 0;

    printf("Writing Program to Flash Chip from memory and reading...\n");
    ...
    if(count == 0){
        ...
        //Set the Address
        FMemAddrByte1 = (FAddr >> 16) & 0xFF; //get the highest byte in the address
        FMemAddrByte2 = (FAddr >> 8) & 0xFF; //get the second byte in the address
        FMemAddrByte3 = FAddr & 0xFF;   //get the lowest byte in the address
        ...
        //Send Page Program Command to Chip
        SPI_Data = 0x02;
        WaitForSPITransmitComplete();
        //Read Garbage Values Once Command Transmission is Done
        x = SPI_Data;

        //Send 24-bit Address
        SPI_Data = FMemAddrByte1;
        WaitForSPITransmitComplete();
        x = SPI_Data;

        SPI_Data = FMemAddrByte2;
        WaitForSPITransmitComplete();
        x = SPI_Data;

        SPI_Data = FMemAddrByte3;
        WaitForSPITransmitComplete();
        x = SPI_Data;
    }
        ...
        //Disable Chip Select to indicate page program completion
        Disable_SPI_CS();

        //Check if writing is complete by polling the Flash Chip Status Register
        WaitWriteCommand();

        //Start Address at the next page
        FAddr += 256;
        count = 0;
        continue;
    }
    ...
    //Disable Chip Select to finalize writing
    Disable_SPI_CS();

    //Poll Flash Chip to make sure writing is done
    WaitWriteCommand();

    printf("\r\n Programming Flash Chip is Done!");
    FlashMemTest();
}

/******************************************************************************************
** Testing if Programming the Flash Memory was Successful
*******************************************************************************************/
void FlashMemTest(void){

    unsigned char *RamPtr;
    unsigned char ReadByte, x;
    unsigned char FMemAddrByte1, FMemAddrByte2, FMemAddrByte3;

     printf("\r\nTesting Flash Chip....");

    //Since Address Increments Automatically, we only need to set starting address to 0
    FMemAddrByte1 = 0;
    FMemAddrByte2 = 0;
    FMemAddrByte3 = 0;
    ...
    for(RamPtr=0x08000000;RamPtr<=0x08040000;RamPtr++){
        //To read, we basically need to send dummy data each cycle and read the output from the Data Register
        SPI_Data = 0xFF;
        WaitForSPITransmitComplete();
        ReadByte = SPI_Data;
        ...
        if(ReadByte != *RamPtr){
            printf("\r\n Flash Test Failed!!");
            printf("\r\n Failed at byte number %d", RamPtr - 0x08000000);
            Disable_SPI_CS();
            return;
        }
    }

    //Disable Chip Select to End this Test
    Disable_SPI_CS();
...
void LoadFromFlashChip(void)
{
    ...
    //Disable Chip Select to End this Test
    Disable_SPI_CS();
    printf("\r\nLoading Program From Flash to Dram: PASSED");
...
void MemoryTest(void)
{
    ...
    char c ;
	char pattern;
	unsigned char FillData1, FillData2, FillData3, FillData4;
	unsigned int preFill;
	unsigned int count = 0;

	// Start of Part B
	printf("\r\nCPEN 412 - Assignment 1 Part B") ;
	// prompt user to enter bytes, words, or long words
	printf("\r\nSelect width of data\r\n1: Bytes\r\n2: Words\r\n3: Long Words\r\n");
	c = getchar();
	// prompt user to enter a choice of 4 patterns to write to data
	printf("\r\nSelect pattern to fill memory with.\r\nOptions are split in 3 depending on the selection of bytes, words, long words respectively:\r\n1: 56, 5678, 56789ABC \r\n2: AB, ABCD, ABCDEF90\r\n3: BE, BEEF, BEEFDEAD \r\n4: 01, 0110, 01100110\r\n");
	pattern = getchar();
	if (pattern == '1'){
		FillData1 = 0x56;
		FillData2 = 0x78;
	...
	// prompt user for a start address
	printf("\r\nEnter Addresses...");
    printf("\r\nStart Address: ") ;
    StartRamPtr = Get8HexDigits(0) ;
	...
	if (c == '2' || c == '3')
	{
		if (StartRamPtr % 2 == 1 || EndRamPtr % 2 == 0) {// address is bad return
			printf("\r\nThe addresses being tested is not valid for words and long words. Exiting...");
			return;
		}
		printf("\r\nStartRamPtr before increment: %x", StartRamPtr);
		StartRamPtr++;
		printf("\r\nStartRamPtr after increment: %x", StartRamPtr);
		// word case
		if (c == '2'){
			while (StartRamPtr < EndRamPtr){
				*StartRamPtr = FillData1; // fills the lower half of the byte
				*(StartRamPtr + 1) = FillData2; // fills upper half of the byte

				if (count % 5000 == 0){
					printf("\r\nAddress being Tested: %x - %x\r\nMemory being written: %x%x", StartRamPtr, StartRamPtr+1, FillData1, FillData2);
				}
				// move by full byte since we are writing a word ???
				StartRamPtr = (StartRamPtr + 2);
				count += 2;
			}
		}
		// long word case
			...
			while (StartRamPtr < EndRamPtr){
				// print progress here
				*StartRamPtr = FillData1 ;
				*(StartRamPtr + 1) = FillData2 ;
				*(StartRamPtr + 2) = FillData3 ;
				*(StartRamPtr + 3) = FillData4 ;
				if (count % 5000 == 0){
					printf("\r\nAddress being Tested: %x - %x\r\nMemory being written: %x%x%x%x", StartRamPtr, StartRamPtr+3, FillData1, FillData2, FillData3, FillData4);
				}

				StartRamPtr = (StartRamPtr + 4) ;
			}
		}
	}
	if (c == '1'){
		while(StartRamPtr < EndRamPtr){
			*StartRamPtr++ = FillData1;
			if (count % 5000 == 0){
				printf("\r\nAddress being Tested: %x\r\nMemory being written: %x", StartRamPtr, FillData1);
			}
			count++;
		}
	}
...
void main(void)
{
	...
	char *NameMessage = "Amir Gorgy - 29650165";
...
void SPI_Init(void)
{
    ...
    //
    //Using 0 for all bits that are reserved or read only
...
void WriteSPIChar(unsigned char c)
{
    ...
    //Load c into Data Register to send to the flash memory
    SPI_Data = c;
    WaitForSPITransmitComplete();
    x = SPI_Data;

    //Since we are only writing 1 byte, we need to immediately disable the chip
    Disable_SPI_CS();

    //Poll Flash Status Register To make sure writing is completed
    WaitWriteCommand();
}
...
void DisableBlockProtect(void){
    ...
    //Send Command To Write Status Register 1
    SPI_Data = 0x01;
    WaitForSPITransmitComplete();
    x = SPI_Data;

    //Disable Block Protection (BP2, BP1, BP0 = 000)
    SPI_Data = 0x00;
    WaitForSPITransmitComplete();
    x = SPI_Data;

    //Disable Chip Select to Conclude this instruction
    Disable_SPI_CS();
}
/************************************************************************************
** Enables Writing to Flash Memory
************************************************************************************/
